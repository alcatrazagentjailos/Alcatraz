...
import os, time, json, ast, traceback
...
class KillSwitch:
    def __init__(self, audit: AuditLog, q: Queue):
        self.audit = audit
        self.q = q
        self.tripped = False
...
# =========================================================
# TOOL GATE — BANKR JAIL (STEP 1 + STEP 2)
# =========================================================
...
class ToolGate:
    ...
    def bankr_prompt(self, prompt: str) -> dict:
        ...
        try:
            ...
            self.audit.write("tool_error", tool="bankr", error=str(e))
            self.kill.trip(f"BANKR_HTTP_ERROR:{e}")

        self.audit.write("tool_ok", tool="bankr")
        return res
