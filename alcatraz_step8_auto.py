#!/usr/bin/env python3
"""ALCATRAZ Step 8 — Human Approval (auto-approve)

This script runs the Step 8 approval flow but automatically approves any
approval request so tests can run non-interactively.
"""
...
# Minimal AuditLog, Capability, KillSwitch and ToolGate similar to step8
class AuditLog:
...
class ToolGate:
    def __init__(self, caps: CapabilitySet, audit: AuditLog, kill: KillSwitch, approval_req_q: Queue, approval_res_q: Queue):
        self.caps = caps
        self.audit = audit
        self.kill = kill
    ...
    def _require_human_approval(self, cap: Capability, prompt: str):
        ...
        # wait for response (auto-approve controller will reply)
        start = time.time()
        while True:
...
# Agent cell and run_agent with auto-approve controller
...
def run_agent_auto_approve(code, grants):
    now = time.time()
    cap_dict = {n: {"expires": now + t, "scope": s} for n, t, s in grants}
    approval_req_q = Queue()
    approval_res_q = Queue()
    result_q = Queue()
    p = Process(target=agent_cell, args=(code, cap_dict, "./audit/alcatraz.jsonl", approval_req_q, approval_res_q, result_q))
    p.start()

    # auto-approve controller: listen for requests and immediately approve
    start = time.time()
    while True:
...
'''
    result = run_agent_auto_approve(
        AGENT_CODE,
        grants=[("bankr.use", 120, {"approval_required": True, "approval_timeout_s": 30, "blocked_actions": ["transfer", "withdraw"], "max_usd": 101})]
    )
    print("\nRESULT:", result)
